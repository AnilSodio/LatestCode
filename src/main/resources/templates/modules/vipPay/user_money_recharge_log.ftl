	...
	<div class="page-container-custom">
        ...
        <div class="portlet light portlet-fit portlet-datatable ">
	       <ul class="nav nav-tabs mb-25">
        	...
        	<@form.form id="searchForm" modelAttribute="vo" action="${ctx}/sys/recharge/log_list" method="post" class="breadcrumb form-search">
					...
					<div>
						...
						<label><@spring.message code="sys.paymentTypes"></@spring.message>：
							<select id="paymentType" name="paymentType" class="select2 form-control input-small">
								<option value=''  ><@spring.message code="stats.total"></@spring.message></opetion>
								<option value='alipay_app'  <#if vo.paymentType ?? && vo.paymentType == 'alipay_app' >selected='selected'</#if>>Alipay</opetion>
								<option value='wx_app' <#if vo.paymentType ?? && vo.paymentType == 'wx_app'>selected='selected'</#if>>WeChat</option>
								<option value='wx_js' <#if vo.paymentType ?? && vo.paymentType == 'wx_js'>selected='selected'</#if>>WeChat applet</option>
								<option value='stripe' <#if vo.paymentType ?? && vo.paymentType == 'stripe'>selected='selected'</#if>>stripe</option>
							<select>
						</label>
						&nbsp;&nbsp;
         ...
         <#if (page.content?size > 0)>
	        <div class="portlet-body">
				...
				<table id="contentTable" class="table table-striped table-bordered table-hover table-checkable order-column dataTable">
					<tr>
			            <th width="6%"><@spring.message code="userInfo.member.vip.orderCode" /></th>
			            <th width="4%"><@spring.message code="financial.deposit.business.order" /></th>
					...
					<#list page.content as info>
						<tr>
				            ...
				            <td>
				            	<#if info.paymentType?? && info.paymentType == 'wx_js'>
									WeChat Applet
								<#elseif info.paymentType ?? && info.paymentType == 'wx_app'>
									Wechat
								<#elseif info.paymentType ?? && info.paymentType == 'alipay_app'>
									Alipay
								<#elseif info.paymentType ?? && info.paymentType == 'stripe'>
									stripe
								<#else>
									${info.paymentType!}
								</#if>
